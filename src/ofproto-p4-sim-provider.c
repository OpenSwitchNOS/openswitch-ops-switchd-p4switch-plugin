...
p4_gre_tunnel_check_config_change(struct ofbundle *bundle, struct netdev *netdev)
{
    ...
    if (tnl_node) {
        if (tnl_node->remote_ip != dest_ip ||
           ...
           if (status == SWITCH_STATUS_SUCCESS) {
                VLOG_DBG("%s: Existing GRE tunnel deleted. Creating new tunnel");
...
bundle_del_port(struct sim_provider_ofport *port)
{
    ...
    if (port == NULL) {
        VLOG_ERR("%s, Invalid port", __func__);
        return;
    }
...
found:     ;
    ...
    if (ofproto->vrf == false) {
        ...
        } else if (strncmp(type, OVSREC_INTERFACE_TYPE_VXLAN, 5) == 0 ||
                   strncmp(type, OVSREC_INTERFACE_TYPE_GRE_IPV4, 8) == 0) {
