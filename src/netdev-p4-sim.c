...
{
    struct netdev_sim *ns = netdev_sim_cast(netdev);
    ovs_assert(is_sim_class(netdev_get_class(netdev)));

    VLOG_DBG("get subinterface vlan as %d\n", ns->subintf_vlan_id);
    *vlan = ns->subintf_vlan_id;
}
...
static int
netdev_sim_subint_update_flags(struct netdev *netdev_,
                                    ...
                                    enum netdev_flags *old_flagsp)
{
    ...
    if (strlen(netdev->parent_netdev_name)) {
        ...
        if (parent != NULL) {
            ...
            parent_flagsp = parent_netdev->flags;
...
static const struct netdev_class sim_subinterface_class = {
    ...
    netdev_sim_subint_update_flags,
