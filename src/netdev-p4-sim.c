...
netdev_sim_rmac_handle_allocate(struct netdev *netdev_)
{
    ...
    if (netdev->rmac_handle == SWITCH_API_INVALID_HANDLE) {
        ...
        status = switch_api_router_mac_add(
                     ...
                     netdev->rmac_handle,
                     &mac);
        VLOG_INFO("rmac_handle_allocate mac %2x:%2x:%2x:%2x:%2x:%2x",
                   netdev->hwaddr[0],
                   netdev->hwaddr[1],
                   netdev->hwaddr[2],
                   netdev->hwaddr[3],
        ...
        if (status != SWITCH_STATUS_SUCCESS) {
            VLOG_INFO("rmac handle allocate failed");
            return EINVAL;
        }
    }
    return 0;
...
netdev_sim_internal_set_hw_intf_info(struct netdev *netdev_, const struct smap *args)
{
    ...
    if (bridge) {
        ...
        if (system(cmd) != 0) {
            VLOG_INFO("NETDEV-SIM | system command failure cmd=%s", cmd);
        }
...
netdev_sim_set_hw_intf_info(struct netdev *netdev_, const struct smap *args)
{
        ...
        if (hw_id) {
            ...
            if (netdev->hostif_handle == SWITCH_API_INVALID_HANDLE) {
                ...
                status = switch_api_packet_net_filter_tx_create(
                             ...
                             &tx_action);
                if (status != SWITCH_STATUS_SUCCESS) {
                    VLOG_INFO("packet net filter tx create failed");
                }
                ...
                status = switch_api_packet_net_filter_rx_create(
                             ...
                             &rx_action);
                if (status != SWITCH_STATUS_SUCCESS) {
                    VLOG_INFO("packet net filter rx create failed");
                }
            }
        } else {
            VLOG_INFO("No hw_id available");
    ...
    if(mac_addr != NULL) {
        ...
        VLOG_INFO("Invalid mac address %s", mac_addr);
    }
...
netdev_sim_set_hw_intf_config(struct netdev *netdev_, const struct smap *args)
{
    ...
    VLOG_INFO("Interface=%s hw_enable=%d ", netdev->linux_intf_name, hw_enable);
    ...
    if (system(cmd) != 0) {
        VLOG_INFO("system command failure: cmd=%s",cmd);
    }
...
netdev_sim_get_subintf_vlan(struct netdev *netdev, switch_vlan_t *vlan)
{
    ...
    ovs_mutex_lock(&dev->mutex);
    VLOG_INFO("get subinterface vlan as %d\n", dev->subintf_vlan_id);
                                    ...
                                    enum netdev_flags *old_flagsp)
{
    ...
    VLOG_INFO("%s parent flags = %d",__FUNCTION__, parent_flags);

    ovs_mutex_lock(&netdev->mutex);
    VLOG_INFO("%s netdev flags = %d",__FUNCTION__, netdev->flags);
...
netdev_sim_subintf_set_config(struct netdev *netdev_, const struct smap *args)
{
    ...
    if (parent_intf_name != NULL) {
        VLOG_INFO("netdev set_config parent interface %s, and vlan = %d",
        ...
        if (parent != NULL) {
            ...
            VLOG_INFO("Parent found, netdev vlan set = %d",
                            vlanid);
            netdev_close(parent);
        }
    }
...
static void
netdev_bcmsdk_vport_destruct(struct netdev *netdev_)
{
    tnl_remove(netdev_);
    netdev_bcmsdk_vport_destruct(netdev_);
}
...
get_tunnel_config(const struct netdev *dev, struct smap *args)
{
    ...
    VLOG_INFO("%s: locking",__func__);
    ovs_mutex_lock(&netdev->mutex);
    tnl_cfg = netdev->tnl_cfg;
    ovs_mutex_unlock(&netdev->mutex);
    VLOG_INFO("%s: unlocking",__func__);
...
set_tunnel_config(struct netdev *dev_, const struct smap *args)
{
    ...
    VLOG_INFO("%s: netdev name = %s type = %s", __func__, name, type);
    ...
    SMAP_FOR_EACH (node, args) {
        if (!strcmp(node->key, "remote_ip")) {
            ...
            VLOG_INFO("%s: remote ip = %s",__func__, node->value);
            switch (err) {
            case ENOENT:
            ...
            VLOG_INFO("%s: local ip = %s",__func__, node->value);
            switch (err) {
            case ENOENT:
            ...
            if (smap_get(args, "certificate")) {
                ...
                if (!use_ssl_cert || strcmp(use_ssl_cert, "true")) {
                    VLOG_INFO("%s: 'peer_cert' requires 'certificate' argument",
                             name);
                    return EINVAL;
                }
                ipsec_mech_set = true;
    ...
    if (tnl_cfg.ipsec) {
        ...
        if (pid < 0) {
            VLOG_INFO("%s: IPsec requires the ovs-monitor-ipsec daemon",
                     name);
            return EINVAL;
        }

        if (smap_get(args, "peer_cert") && smap_get(args, "psk")) {
            VLOG_INFO("%s: cannot define both 'peer_cert' and 'psk'", name);
            return EINVAL;
        }

        if (!ipsec_mech_set) {
            VLOG_INFO("%s: IPsec requires an 'peer_cert' or psk' argument",
                     name);
            return EINVAL;
        }
    }
    ...
    if (!ipv6_addr_is_set(&tnl_cfg.ipv6_dst) && !tnl_cfg.ip_dst_flow) {
        VLOG_INFO("%s: %s type requires valid 'remote_ip' argument",
                 name, type);
        return EINVAL;
    }
    if (tnl_cfg.ip_src_flow && !tnl_cfg.ip_dst_flow) {
        VLOG_INFO("%s: %s type requires 'remote_ip=flow' with 'local_ip=flow'",
                 name, type);
        return EINVAL;
    }
    if (src_proto && dst_proto && src_proto != dst_proto) {
        VLOG_INFO("%s: 'remote_ip' and 'local_ip' has to be of the same address family",
                 name);
        return EINVAL;
    }
    if (!tnl_cfg.ttl) {
        tnl_cfg.ttl = DEFAULT_TTL;
        VLOG_INFO("%s: Setting default TTL",__func__);
    }
    ...
    if(tnl_cfg.in_key_present)
        VLOG_INFO("set_tunnel_config key %lx",
                (unsigned long int)ntohll(tnl_cfg.in_key));

    VLOG_INFO("%s: in key  = %d, out key = %d",__func__, tnl_cfg.in_key, tnl_cfg.out_key);
    ovs_mutex_lock(&dev->mutex);
    ...
    dev->state = TNL_INIT;
    VLOG_INFO("%s: Finished setting configuration",__func__);
    return 0;
...
tunnel_get_status(const struct netdev *netdev_, struct smap *smap)
{
    ...
    VLOG_INFO("%s: netdev->link_state = %d",__func__, netdev->link_state);
