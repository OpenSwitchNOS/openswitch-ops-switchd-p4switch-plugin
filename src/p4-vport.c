...
logical_nw_lookup_vni(uint32_t vni)
{
    ...
    HMAP_FOR_EACH_WITH_HASH(node, hmap_t_node, hash, &logical_nw_hmap) {
        if (node->vni == vni) {
            VLOG_INFO("%s, found node with vni = %d, handle = %d",__func__,
                        node->vni, node->logical_nw_handle);
            return node;
        }
    }
...
get_logical_nw_handle(uint32_t vni)
{
    ...
    if (node){
        VLOG_INFO("%s, after found node with vni = %d, handle = %d",__func__,
                        node->vni, node->logical_nw_handle);
        return node->logical_nw_handle;
    }
    VLOG_INFO("%s:logical_network lookup failed",__func__);
...
logical_nw_remove(uint32_t vni)
{
    ...
    if(node) {
        ...
        VLOG_INFO("%s:logical_network removed",__func__);
        free(node);
    }
}
...
p4_vport_delete_tunnel(struct ofbundle *bundle, struct netdev *netdev)
{
    ...
    if (!tnl_node) {
        ...
        VLOG_INFO("%s VNI = %d", __func__, vni);
        ...
        p4_ln_handle = get_logical_nw_handle(vni);
        VLOG_INFO("%s Logical nw = %d", __func__, p4_ln_handle);
        switch_api_logical_network_member_remove(DEFAULT_P4_DEVICE, p4_ln_handle,
                                                 tnl_node->tunnel_handle);
...
p4_vport_bind_access_port(int hw_unit, p4_pbmp_t pbm, int vni, int vlan)
{
    ...
    P4_PBMP_ITER (pbm, aport) {
        ...
        if (ln_node != NULL) {
            ...
            if (P4_HANDLE_IS_VALID(p4_ln_handle)) {
                ...
                VLOG_INFO("%s : unable to find logical network handle", __func__);
            }
        } else {
            VLOG_INFO("%s : unable to find logical network node", __func__);
        }
    }
    return 0;
}
