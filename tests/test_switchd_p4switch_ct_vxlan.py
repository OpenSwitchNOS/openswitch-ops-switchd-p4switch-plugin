...
class vxlanTest( OpsVsiTest ):
    ...
    def vtep_setup(self,ip1,ip2, br="bridge_normal",vni = "flow"):
        ...
        s1.ovscmd(OPS_VSCTL + " add-port %s vtep -- set interface vtep type=vxlan options:remote_ip=%s options:local_ip=%s options:key=%s" % (br,ip1,ip2,vni))
        sleep(10)
        s2.ovscmd(OPS_VSCTL + " add-port %s vtep -- set interface vtep type=vxlan options:remote_ip=%s options:local_ip=%s options:key=%s" % (br,ip2,ip1, vni))
        '''
    ...
    def log_sw_create(self, unit, vni):
        info("\n ### Logical Sw create ### \n")
        for sw in self.net.switches:
            sw.ovscmd(OVS_APPCTL + " lsw %s %s" %(unit, vni))
