...
class vxlanTest( OpsVsiTest ):
    ...
    def vtep_setup(self,ip1,ip2, br="bridge_normal",vni = "flow"):
        ...
        s1.cmdCLI("interface tunnel 10 mode vxlan")
        s1.cmdCLI("source ip %s" % ip2)
        s1.cmdCLI("destination ip %s" % ip1)
        s1.cmdCLI("vxlan-vni %s" % vni)
        s1.cmdCLI("exit")

        sleep(10)
        s2.cmdCLI("configure terminal")
        s2.cmdCLI("interface tunnel 10 mode vxlan")
        s2.cmdCLI("source ip %s" % ip1)
        s2.cmdCLI("destination ip %s" % ip2)
        s2.cmdCLI("vxlan-vni %s" % vni)
        s2.cmdCLI("exit")
        ops = s1.ovscmd(OPS_VSCTL + " show")
        ovs = s2.ovscmd(OPS_VSCTL + " show")
        info("\n \n ops-ctl \n %s \n\n ovs-ctl \n%s \n\n" % (ops, ovs))
    ...
    def conf_net_port(self, intf):
       ...
       info("\nRouting switch s1\n")
       s1.cmdCLI("configure terminal")
       s1.cmdCLI("ip route 9.0.2.0/24 3")
       s1.cmdCLI("do show run")
       s2.cmdCLI("configure terminal")
       s2.cmdCLI("ip route 9.0.1.0/24 3")
       s2.cmdCLI("do show run")
       s2.cmdCLI("exit")
       sleep(5)
       s1.ovscmd("ip netns exec swns ping -c 2 9.0.2.2")

    def log_sw_create(self, unit, vni):
        info("\n ### Logical Sw create ### \n")
        for sw in self.net.switches:
            sw.cmdCLI("configure terminal")
            sw.cmdCLI("vni %s" % vni)
            sw.cmdCLI("do show run")
            sw.cmdCLI("exit")

    def vlan_to_vni_mapping(self, unit, vlan, vni):
        info("\n ### VLAN to VNI mapping ### \n")
        for sw in self.net.switches:
            sw.cmdCLI("configure terminal")
            sw.cmdCLI("vxlan vlan %s vni %s" % (vlan, vni))
            sw.cmdCLI("do show run")
            sw.cmdCLI("exit")
    ...
    def vxlan_prep(self):
        info("\n\n\n ########## testing L2 vxlan  ##########\n\n\n")
        self.tcpdump_setup()
        self.conf_access_port("1", "21")
        self.conf_net_port("3")

    def vxlan(self):
        self.log_sw_create("0", "1760")
        self.vlan_to_vni_mapping("0", "21", "1760")
        self.vtep_setup(SW_ROUTERIP[1], SW_ROUTERIP[0], "bridge_normal", "1760")
...
class Test_switchd_container_vlan_access:
    ...
    def test_switchd_container_check_config(self):
        #pause to setup log
        CLI(self.test.net)
        self.test.vxlan_prep()
        #pause to resolve nexthop
        CLI(self.test.net)
        self.test.vxlan()
        CLI(self.test.net)
        self.test.test_ping()
